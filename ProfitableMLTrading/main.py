# Profitable ML-Enhanced Trading Algorithm
from AlgorithmImports import *
import joblib
import numpy as np
from pathlib import Path

class ProfitableMLTrading(QCAlgorithm):
    def Initialize(self):
        self.SetStartDate(2020, 1, 1)  # Recent data for better performance
        self.SetCash(10_000)
    ...
    def load_ml_model(self):
        ...
        try:
            ...
            model_paths = [
                "/Users/admin/Developer/AutoTrader/research/models/lgb_spy_2025-10-02.pkl",
                "research/models/lgb_spy_2025-10-02.pkl",
                "../research/models/lgb_spy_2025-10-02.pkl"
            ]
            ...
            for model_path in model_paths:
                if Path(model_path).exists():
                    ...
                    self.Debug(f"Loaded ML model: {model_path}")
                    return
                    
            self.Debug("No ML model found, using technical indicators only")
    ...
    def calculate_features(self, current_price: float) -> dict:
        ...
        # Get recent prices (filter out None values)
        recent_prices = []
    ...
    def OnData(self, data: Slice):
        ...
        # More aggressive trading logic for better profitability
        # Lower thresholds to generate more trades
        should_buy = (ml_signal > 0.55 and sma_bullish) or \
                    (ml_signal > 0.52 and sma_bullish and macd_bullish) or \
                    (ml_signal > 0.50 and sma_bullish and rsi_oversold)
        
        should_sell = (ml_signal < 0.45) or \
                     (ml_signal < 0.48 and rsi_overbought) or \
                     (ml_signal < 0.50 and not sma_bullish)
